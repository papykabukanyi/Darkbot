"""
Main scripfrom config import (WEBSITES, MIN_DISCOUNT_PERCENT, BRANDS_OF_INTEREST,
                   SAVE_TO_CSV, CSV_FILENAME, DATABASE_ENABLED, DATABASE_PATH,
                   EMAIL_INTERVAL_MINUTES, EMAIL_NOTIFICATIONS,
                   SNEAKER_SITES, DEFAULT_SITES, MARKET_PRICE_SITES)r the sneaker deal finder bot.
"""
...
from storage import DealStorage, R2Storage
...
def main():
    ...
    parser.add_argument('--no-schedule', action='store_true', help='Run once instead of continuously')
    parser.add_argument('--interval', type=int, default=30,
                        help='Interval in minutes for scheduled runs (default: 30)')
    ...
    # By default, we run in continuous mode every 30 minutes
    if not args.no_schedule:
        ...
        def scheduled_job():
            ...
            print(f"\n{colorama.Fore.GREEN}[{current_time}] Running scheduled scan...{colorama.Style.RESET_ALL}")
            run_scraper_job(args, scheduled_run=True)
        
        # Schedule to run every X minutes (default 30)
        schedule.every(args.interval).minutes.do(scheduled_job)
        
        print(f"\nBot is now running on a schedule every {args.interval} minutes.")
        ...
        try:
            ...
            while True:
                ...
                # Display a heartbeat every 5 minutes
                if int(time.time() - start_time) % 300 == 0:
                    uptime_minutes = int((time.time() - start_time) / 60)
                    sys.stdout.write(f"\rBot running - Uptime: {uptime_minutes} minutes - Next scan in: {schedule.idle_seconds()//60:.0f} minutes")
                    sys.stdout.flush()
