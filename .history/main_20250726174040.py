...
def run_scraper(scraper_class, site_config, category='sale'):
    """
    ...
    try:
        with scraper_class(site_config) as scraper:
            logger.info(f"Scraping {scraper.name}...")
            site_deals = scraper.scrape_site()
            ...
            if site_deals:
                ...
                logger.info(f"Found {len(filtered_deals)} deals on {scraper.name} meeting criteria")
                deals.extend(filtered_deals)
    
    except Exception as e:
        logger.error(f"Error running scraper for {site_config.get('name', 'unknown site')}: {e}")
    ...
    for site_name in tqdm(args.sites, desc="Scraping websites"):
        if site_name in WEBSITES:
            ...
            try:
                ...
                site_deals = run_scraper(scraper_class, site_config)
